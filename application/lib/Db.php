...
//        'db'        => '',
    ...
    function __construct($opt = array())
    {
        ...
        $db_config = include ('application/config/db_config.php');

        $this->defaults['host'] = $db_config['host'];
        $this->defaults['user'] = $db_config['user'];
        $this->defaults['pass'] = $db_config['pass'];
        $this->defaults['db'] = $db_config['db'];
        ...
        if ( !$this->conn )
        {
            if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                ...
                $arrayError = array();
                $arrayError['status'] = 'error';
                $arrayError['message'] = 'serverdown';
                print_r(json_encode($arrayError));
            } else {
                global $global_puth, $puth;
                if($puth == '/hello.png') {
                    header('Content-Type: image/gif');
                    readfile($_SERVER['DOCUMENT_ROOT'].'/assets/images/tracking.gif');
                } else {
//                    include $global_puth.'tmpl/500.php';
                }
            }
